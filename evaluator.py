...
def policy(state, explore=0.0):
    ...
    if np.random.random() <= explore:
        action = int(np.random.randint(low=0, high=4, size=1))  # Changed high to 4
    return action
...
for episode in range(NUM_EPISODES):
    ...
    state, _ = cliffEnv.reset()  # Resetting state correctly
    ...
    while not done:
        ...
        state, reward, done, truncated, info = cliffEnv.step(action)  # Updated unpacking
